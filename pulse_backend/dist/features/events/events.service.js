...
export class EventsService {
    ...
    async getEventBySlug(slug, userId) {
        const event = await prisma.event.findUnique({
            where: { slug },
            include: {
                organization: {
    ...
    async searchEvents(filters) {
        const { q, eventType, city, country, startDate, endDate, isFree, minPrice, maxPrice, categoryId, tags, featured, latitude, longitude, radius = 50, page = 1, limit = 20, sortBy = 'startTime', } = filters;
        const where = {
            status: 'PUBLISHED',
        };
        // Text search
